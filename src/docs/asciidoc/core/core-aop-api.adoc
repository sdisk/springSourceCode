...
The `org.springframework.Pointcut` interface is the central interface, used to
...
`org.springframework.Pointcuts` class or by using the
...
`org.springframework.AspectJExpressionPointcut`. This is a pointcut that
...
`org.springframework.JdkRegexpMethodPointcut` is a generic regular
...
----
	<bean id="settersAndAbsquatulatePointcut"
			class="org.springframework.JdkRegexpMethodPointcut">
...
----
	<bean id="settersAndAbsquatulateAdvisor"
			class="org.springframework.RegexpMethodPointcutAdvisor">
...
are specified by using the `org.springframework.ControlFlowPointcut` class.
...
`org.springframework.ThrowsAdvice` interface does not contain any methods. It is a
...
`org.springframework.AfterReturningAdvice` interface, which the following listing shows:
...
case, we extend the `org.springframework.DelegatingIntroductionInterceptor`
...
`org.springframework.DefaultPointcutAdvisor` is the most commonly used
...
`org.springframework.ProxyFactoryBean`. This gives complete control over
...
Some key properties are inherited from `org.springframework.ProxyConfig`
...
----
	...
	<bean id="debugInterceptor" class="org.springframework.DebugInterceptor">
	</bean>
...
----
	...
	<bean id="person" class="org.springframework.ProxyFactoryBean">
		<property name="proxyInterfaces" value="com.mycompany.Person"/>
...
----
	...
	<bean id="global_debug" class="org.springframework.DebugInterceptor"/>
	<bean id="global_performance" class="org.springframework.PerformanceMonitorInterceptor"/>
----
====
...
`org.springframework.ProxyFactory`. You can create this with a target
...
`org.springframework.Advised` interface. Any AOP proxy can be cast to this
...
[subs="verbatim,quotes"]
----
	<bean class="org.springframework.BeanNameAutoProxyCreator">
...
`org.springframework.TargetSource` interface. This interface is responsible for
...
The `org.springframework.HotSwappableTargetSource` exists to let the target
...
----
	...
	<bean id="swapper" class="org.springframework.HotSwappableTargetSource">
		<constructor-arg ref="initialTarget"/>
	</bean>

	<bean id="swappable" class="org.springframework.ProxyFactoryBean">
...
`org.springframework.AbstractPoolingTargetSource` to support any other
pooling API.
...
----
	...
	<bean id="poolTargetSource" class="org.springframework.CommonsPool2TargetSource">
		<property name="targetBeanName" value="businessObjectTarget"/>
		<property name="maxSize" value="25"/>
	</bean>

	<bean id="businessObject" class="org.springframework.ProxyFactoryBean">
...
`org.springframework.PoolingConfig` interface, which exposes information
...
[subs="verbatim,quotes"]
----
	<bean id="prototypeTargetSource" class="org.springframework.PrototypeTargetSource">
...
[subs="verbatim,quotes"]
----
	<bean id="threadlocalTargetSource" class="org.springframework.ThreadLocalTargetSource">
		<property name="targetBeanName" value="businessObjectTarget"/>
	</bean>
----
====
