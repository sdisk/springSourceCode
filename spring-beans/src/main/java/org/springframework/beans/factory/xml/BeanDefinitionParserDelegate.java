...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		if (beanDefinition != null) {
			if (!StringUtils.hasText(beanName)) {
				try {
					//如果不存在beanName，那么根据spring的命名规则为当前bean生成对应的beanName
					if (containingBean != null) {
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		try {
			...
			//BeanDefinitionReaderUtils  GenericBeanDefinition bd = new GenericBeanDefinition();
...
//	</bean>
	 ...
	 *
	 * <beans>
	 *     <!-- 默认情况是按照参数的顺序注入，当指定了index可以改变顺序-->
	 ...
	 * 解析子元素lookup-method
	 * 获取器注入，将方法声明为返回某个类型的bean，但是实际返回的bean是在配置文件中配置的，实现了动态可插拔
	 *
	 ...
	 * 在运行时用新的方法替换旧的方法，不但可以动态的替换返回的bean，还可以动态的改变原来方法的逻辑
	 *
	...
	public Object parsePropertyValue(Element ele, BeanDefinition bd, @Nullable String propertyName) {
		...
		// Should only have one child element: ref, value, list, etc.只有一个类型
		NodeList nl = ele.getChildNodes();
		...
		//<constructor-arg ref = "hello" /> RuntimeBeanReference处理
		if (hasRefAttribute) {
		...
		//<constructor-arg value = "hello" /> TypedStringValue处理
	...
	public BeanDefinitionHolder decorateBeanDefinitionIfRequired(Element ele, BeanDefinitionHolder definitionHolder) {
		//第三个参数时父类的bean，当对某个嵌套就行分析时，这里需要传递父类的beanDefinition
		//这里使用父类的scope属性，以备子类若没有设置scope时默认使用父类的属性，这里分析的是顶层的配置，所以传递null
